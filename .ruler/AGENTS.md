...
**Note:** This project uses **pnpm** as the package manager. All commands should use `pnpm nx` instead of `pnpm nx`.

```bash
pnpm nx run @nx-test/nx-test:dev     # Start dev server at http://localhost:3000
pnpm nx run @nx-test/nx-test:start   # Serve production build
```

## Building
```bash
pnpm nx run @nx-test/nx-test:build   # Production build
```

## Testing
```bash
pnpm nx run @nx-test/nx-test:test              # Run unit tests (Jest)
pnpm nx run @nx-test/nx-test:test --watch      # Run tests in watch mode
pnpm nx run @nx-test/nx-test-e2e:e2e           # Run E2E tests (Playwright)
pnpm nx run-many -t test                       # Run tests for multiple projects
```

## Linting & Formatting
```bash
pnpm nx run @nx-test/nx-test:lint    # Lint with ESLint (includes @nx/enforce-module-boundaries)
...
## Type Checking
```bash
pnpm nx run @nx-test/nx-test:typecheck   # Run TypeScript type checking
```
...
pnpm nx affected:graph               # Visualize affected projects
```

## Project Management
```bash
pnpm nx graph                        # View interactive project graph
pnpm nx show project @nx-test/nx-test  # Show project details
